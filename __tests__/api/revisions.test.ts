...
vi.mock('@/app/lib/contentStore', () => ({ updateContent: vi.fn() }));
import { updateContent } from '@/app/lib/contentStore';
vi.mock('@/app/lib/documentStore', () => ({ updateDocument: vi.fn() }));
import { updateDocument } from '@/app/lib/documentStore';
...
describe('POST /api/revisions/[id]/restore', () => {
  ...
  it('restores a document revision', async () => {
    vi.mocked(getRevision).mockResolvedValue({
