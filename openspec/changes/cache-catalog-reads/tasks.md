...
- [ ] 4.2 **(ยังไม่ได้ทำ — ต้องรันแอปจริง)** ยืนยันด้วยมือ: แก้ชื่อสินค้าในหน้า
      แอดมิน → เปิดหน้า `/showcase/[id]` ที่ผูกกับสินค้านั้น → ต้องเห็นชื่อใหม่
      **ทันที** ไม่ต้องรอ 5 นาที
- [ ] 4.3 **(ยังไม่ได้ทำ — ต้องรันแอปจริง)** ยืนยันด้วยมือ: ซ่อนสินค้า
      (unpublish) → หน้า showcase ของ content ที่ผูกอยู่ต้อง 404 สำหรับคนที่
      ไม่ได้ login ทันที (กฎเดิมจาก `isHiddenFromAnonymous` ต้องไม่ถูกแคชบัง)
