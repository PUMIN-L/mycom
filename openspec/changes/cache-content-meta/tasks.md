...
- [ ] 6.1 แก้บทความในหน้าแอดมิน → เปิด `/showcase/[id]` → เห็นเนื้อหาใหม่ทันที
- [ ] 6.2 สร้างบทความใหม่ → เปิด `/sitemap.xml` → ต้องมี URL ใหม่โผล่ทันที
- [ ] 6.3 **สถานการณ์ความเสี่ยงหลัก**: hard delete สินค้าที่มี content ผูกอยู่ →
      เปิด `/sitemap.xml` → **ต้องไม่มี** `/showcase/{id}` ของ content ที่ถูกลบ
- [ ] 6.4 กู้คืนบทความจากหน้า revisions → เปิดหน้า showcase → เห็นเวอร์ชันที่กู้ทันที
