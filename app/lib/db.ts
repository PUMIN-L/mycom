  ...
  if (!globalForDb._dbInitialized) {
    ...
    try {
      ...
      for (const p of seedProducts) {
        await connection.query(
          "INSERT IGNORE INTO products (id, categoryId, image, title_th, title_en, title_zh, desc_th, desc_en, desc_zh, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
          [p.id, p.categoryId, p.image, p.title_th, p.title_en, p.title_zh, p.desc_th, p.desc_en, p.desc_zh, now]
        );
      }
