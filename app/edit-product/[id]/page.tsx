...
export default function EditProduct({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        setBestSellerRank(data.bestSellerRank ?? "");
        setShowBestSellerBadge(data.showBestSellerBadge !== false);
      } catch (err) {
  ...
  /**
   * This product may point at a category that is no longer in the list (the
   * category was deleted, or the categories fetch failed). SearchableDropdown
   * shows its placeholder for a value it has no option for, which would read as
   * "nothing chosen" and hide the link the product still carries — and the save
   * below sends that same id straight back. Prepending a synthetic option keeps
   * it visible and labelled — same trick as `app/customers/EquipmentTab.tsx`
   * (~line 279). Kept identical to `create-product/page.tsx`.
   */
  if (
                ...
                options={[
                  ...
                  ...Array.from({ length: 50 }, (_, i) => {
                    ...
                    return {
                      value: String(rank),
                      label: `อันดับ ${rank}`,
                      subLabel: existing ? (existing.id === productId ? "(อันดับปัจจุบัน)" : `ใช้งานอยู่โดย: ${stripHtml(existing.title_th || existing.title_en || "ไม่มีชื่อ")}`) : "ว่าง",
                      disabled: !!existing && existing.id !== productId
                    };
                  })
                ]}
