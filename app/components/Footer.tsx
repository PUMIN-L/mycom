...
export default function Footer({ email, phone, address, maintenanceOn }: FooterProps) {
  ...
  const quickLinks = [
    ...
    ...(isLoggedIn ? [{ href: "/adminpanel", label: "Content" }] : []),
  ];
  ...
  return (
    <footer className="bg-[#2d2e38] text-white pt-24 pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-20">
          ...
          <div>
            ...
            <div className="space-y-6 text-white/70 text-base font-light">
              {hideContact ? (
                <p className="leading-relaxed text-white/40 italic">ข้อมูลการติดต่อไม่พร้อมใช้งานในขณะนี้</p>
              ) : (
                <>
          ...
          {/* Social/CTA Column — hide LINE button during maintenance */}
          <div>
            <h3 className="text-xl font-bold uppercase tracking-[0.4em] text-[var(--accent)] mb-8">
              Connect
            </h3>
            <div className="flex gap-4">
              {!hideContact && (
                <button
                  ...
                  className="w-12 h-12 border border-white/20 flex items-center justify-center hover:bg-white hover:text-footer transition-all cursor-pointer"
                  aria-label="LINE"
                >
      ...
      {/* Only reachable through the LINE button above, which is itself hidden
          while maintenance mode is on — so this cannot become a way around that. */}
      <LineQrModal isOpen={isLineModalOpen} onClose={closeLineModal} />
    </footer>
  );
}
