      ...
      >
        ...
        >
          <p className="text-sm leading-relaxed text-gray-600 bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3 wrap-break-word">
            ...
            (อยู่คนละหน้า กดปุ่มท้ายฟีดเพื่อไปดู) — ตัวเลขเกณฑ์ทุกตัวในคู่มือนี้ดึงมาจากค่าที่ระบบใช้จริงในการโหลดรอบนี้
          </p>
          ...
          <GuideSection
            ...
            appears={
              <>
                เมื่อ <strong>ใบแจ้งหนี้</strong> ที่ยังเก็บเงินไม่ครบ ใกล้ถึงหรือเลย{" "}
                <strong>วันครบกำหนดชำระ</strong> แล้ว โดยจะขึ้นล่วงหน้า{" "}
                <Val>{RECEIVABLE_ALERT_LEAD_DAYS} วัน</Val> ก่อนถึงกำหนด
                เพื่อให้โทรตามได้ก่อนที่จะเลยกำหนดจริง
              </>
            }
            rule={
              <ul className="space-y-1.5 mt-1">
                <li className="wrap-break-word">
                  นับเฉพาะ <strong>ใบแจ้งหนี้</strong> เท่านั้น —{" "}
                  <strong>ใบวางบิล</strong> เป็นใบปะหน้าไปเก็บเงิน หนี้ก้อนเดียวกันกับใบแจ้งหนี้
                  ถ้านับด้วยจะกลายเป็นหนี้ซ้ำสองเท่า และ <strong>ใบเสร็จ</strong>{" "}
                  คือหลักฐานว่าเงินเข้าแล้ว จึงเป็นตัว <strong>ลด</strong> หนี้ ไม่ใช่สร้างหนี้
                </li>
                <li className="wrap-break-word">
                  ถ้าลูกค้ารายไหนวางบิลอย่างเดียวโดยไม่ได้ออกใบแจ้งหนี้ ให้กด{" "}
                  <strong>&quot;นับเป็นลูกหนี้&quot;</strong> ที่หน้าลูกหนี้ค้างชำระ
                  ระบบจะไม่เดาให้เอง
                </li>
                <li className="wrap-break-word">
                  วันครบกำหนดมาจาก <strong>วันที่บนเอกสาร + เครดิตเทอม</strong> (ค่าเริ่มต้น{" "}
                  <Val>{DEFAULT_CREDIT_TERM_DAYS} วัน</Val> แก้ได้ที่หน้าตั้งค่า)
                  และแก้เป็นรายใบได้ตอนสร้างเอกสาร
                </li>
                <li className="wrap-break-word">
                  <strong>เอกสารที่ยังไม่ได้กำหนดวันครบกำหนด จะไม่ขึ้นเตือนเลย</strong> —
                  ถือว่ายังไม่ได้ตกลงเครดิตกัน ไม่ใช่เลยกำหนด
                  ดูและกำหนดย้อนหลังได้ที่หน้าลูกหนี้ค้างชำระ
                </li>
                <li className="wrap-break-word">
                  รายการนี้ <strong>แสดงสูงสุด</strong> <Val>{ALERT_LIST_DISPLAY_LIMIT}</Val>{" "}
                  รายการต่อครั้ง แต่ตัวเลขบนแท็บคือยอดจริงทั้งหมด
                </li>
              </ul>
            }
            clears={[
              <>
                <strong>บันทึกรับชำระ</strong> จนครบยอด (กดปุ่มบนการ์ดได้เลย ไม่ต้องเปิดเอกสาร)
              </>,
              <>ยกเลิกเอกสารนั้น</>,
              <>เลื่อนแจ้งเตือน (ชั่วคราว) — เช่น ลูกค้าบอกว่าจะโอนวันศุกร์</>,
            ]}
            note={
              <>
                หมวดนี้ <strong>ไม่ใช่</strong> หมวด &quot;เอกสารค้าง&quot; ด้านบน —
                หมวดนั้นดูว่ายังไม่ได้กรอก &quot;เลขที่เอกสาร&quot; ลงในใบขาย
                ส่วนหมวดนี้ดูว่า &quot;เงินยังไม่เข้า&quot;
              </>
            }
          />

          {/* ── 8. เลื่อนแจ้งเตือน (18.8) ───────────────────────────────── */}
          <GuideSection
            icon="⏱️"
          ...
          <GuideSection
            ...
            appears={
              <>
                ...
                <br />
                <strong>อยู่คนละหน้ากับที่นี่แล้ว:</strong> กดปุ่ม “ไปที่หน้าสิ่งที่ต้องทำ”
                ท้ายฟีดแจ้งเตือนด้านล่าง (หรือไปที่ /crm/tasks ตรงๆ) เพื่อเปิดกระดานนี้
              </>
            }
            rule={
              <>
